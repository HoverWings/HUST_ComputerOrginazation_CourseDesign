...
module TopLayer(
    ...
    wire [15:0]ins16_2, ins16_3, ins16_4, ins16_5;
    wire lui2,lui3,lui4,lui5;
    ...
    MEM_WB _mem_wb(
        ...
        .pc4_out(pc4_5),.aluR_out(aluR_5),
         .lui(lui4),.ins16(ins16_4),.lui_out(lui5),.ins16_out(ins16_5)
        );
    ctrl5 _ctrl5(
    ...
    EX_MEM _ex_mem(
        ...
        .lui(lui3),.ins16(ins16_3),.lui_out(lui4),.ins16_out(ins16_4)
        );
    syscall _sys(
    ...
    RegFiletoALU _regtoalu(
        ...
        .regDatain(regdata),.aluR4(aluR_4),.lui4(lui4),.ins16_4(ins16_4),
    ...
    branch_halt _branch(
        .beq(beq3),.bne(bne3),.bltz(bltz3),.syscall(syscall3),
    ...
    IR_Extern _ir_ext(
        ...
        .ins16_32(ins16_32),.ins(ins),.ins26(ins26),.ins16(ins16_2)
    );
    ID_EX _ID_EX(
        ...
        .lui(lui2),.ins16(ins16_2),.lui_out(lui3),.ins16_out(ins16_3)
        );
    control _control(
        .op(op),.func(func),.jump(jump2),.jal(jal2),
        .jr(ret2),.beq(beq2),.bne(bne2),.bltz(bltz2),
        .unsign(unsign2),.RegDst(regdst2),.RegDin(regdin2),
        .RegWrite(regwrite2),.store(dmwrite2),.AluB(alub2),
        .shift(shift2),.syscall(syscall2),.ra_r(ra_r),
        .rb_r(rb_r),.mode(mode2),.aluop(aluop2),.lui(lui2)
        );
    ctrl2 _ctrl2(
        .jump(jump2),.jal(jal2),.ret(ret2),.beq(beq2),
        .bne(bne2),.bltz(bltz2),.RegDst(regdst2),
    ...
    RegFileWrite _regwrite(
        .RegDin5(regdin5),.jal5(jal5),.aluR5(aluR_5),.lui5(lui5),.ins16_5(ins16_5),
