module TopLayer(
    rst,CLK,go,mem_addr,dis_mode,swi_halt,swi_freq,seg,an
     );
     input rst;
     input CLK;
     input go;
     ...
     wire halt;
     halt _halt(
        .syscall(syscall),.equal(equal),.swi_halt(swi_halt),.halt(halt),.go(go)
        );
     display2 _display2(
     ...
     controller _controller(
        ...
        .ALUOP(ALUOP),.syscall(syscall),.RegWrite(RegWrite),.MemWrite(MemWrite),
        .mode(mode),.bat(bat),.AluB1(AluB1),.notext(notext),.lui(lui),.v(v)
        );
     regfile _regfile(
     ...
     extend _entend(
        ._16(ins16),.sha(shamt),.ra(reg1),.v(v),.out1(ins16_32),.out2(ins),.shift(shift),.notext(notext)
        );
     IM _IM(
        .pc(pc),.data(instru)
        );
     ...
     DM _DM(
        .clk(clk),.we(MemWrite),.addr(aluR),.Din(reg2),.mode(mode),
