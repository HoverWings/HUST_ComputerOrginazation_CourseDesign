module DM(clk,addr,visin,Din,mode,we,Dout,visout);
    ...
    always @(*) begin
        if(mode[2])  Dout[31:24]=memory[addr1][31:24];
        else Dout[31:24]=8'b00000000;
        if(mode[2])  Dout[23:16]=memory[addr1][23:16];
        else Dout[23:16]=8'b00000000;
        if(mode[2])  Dout[15:8]=memory[addr1][15:8];
        else Dout[15:8]=8'b00000000;
        if((mode[0]&(addr[1:0]==2'b00))|mode[2])  Dout[7:0]=memory[addr1][7:0];
        else if(mode[0]&(addr[1:0]==2'b01)) Dout[7:0]=memory[addr1][15:8];
        else if(mode[0]&(addr[1:0]==2'b10)) Dout[7:0]=memory[addr1][23:16];
        else if(mode[0]&(addr[1:0]==2'b11)) Dout[7:0]=memory[addr1][31:24];
        else Dout[7:0]=8'b00000000;
    end
    
    assign str4=we;//we & mode[3];
    assign str3=we;//we & mode[2];
    assign str2=we;//we & mode[1];
    assign str1=we;//we & mode[0];
